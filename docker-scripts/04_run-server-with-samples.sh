...
#
# Launches `ghcr.io/groupdocs-merger/merger-net-mcp` as an interactive MCP
...
#                           advertise both `merge` and `split`.
#   --license PATH          Mount a GroupDocs license file (enables
#                           split; without it the server runs in
#                           evaluation mode)
...
IMAGE_REPO="ghcr.io/groupdocs-merger/merger-net-mcp"
...
# nothing to /data when only running merge / non-licensed split.
...
if [ -n "$LICENSE_PATH" ]; then
    ...
    log_info "License mounted — split will work in licensed mode"
else
    log_info "No license — split will return evaluation-mode error (merge works fine)"
fi
...
if [ "$RUN_MODE" = "smoke" ]; then
    ...
    if echo "$response" | grep -q '"merge"' && echo "$response" | grep -q '"split"'; then
