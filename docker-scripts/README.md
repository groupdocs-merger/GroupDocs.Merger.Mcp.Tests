...
Complete bash script suite for running GroupDocs.Merger.Mcp integration tests in both local and Docker environments. **Works on Windows (Git Bash/WSL), Linux, and macOS.**

## 📋 Overview

This folder contains production-ready bash scripts for testing the GroupDocs.Merger.Mcp NuGet package across all scenarios:

- **ToolDiscovery** — MCP server handshake and tool listing (3 tools)
- **Merge** — Combine 2–4 documents into a single file (DOCX, PDF)
- **Split** — Extract specific pages from a document into separate files
- **GetDocumentInfo** — File type, page count, size, per-page dimensions as JSON
...
docker build -t merger-net-mcp:local ../../../GroupDocs.Merger.Mcp/
...
# Docker utilities
docker_pull_image "ghcr.io/groupdocs-merger/merger-net-mcp:latest"
check_docker_daemon
...
- [GroupDocs.Merger.Mcp NuGet](https://www.nuget.org/packages/GroupDocs.Merger.Mcp)
- [Integration Tests Guide](../how-to/06-run-integration-tests.md)
- [Docker Setup Guide](../how-to/02-run-via-docker.md)
- [Test Project](../src/GroupDocs.Merger.Mcp.Tests/GroupDocs.Merger.Mcp.Tests.csproj)
