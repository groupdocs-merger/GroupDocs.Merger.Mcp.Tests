...
Two install routes from [`nuget.org/packages/GroupDocs.Merger.Mcp`](https://www.nuget.org/packages/GroupDocs.Merger.Mcp):
...
```bash
dnx GroupDocs.Merger.Mcp --yes                # latest stable, refreshed every run
dnx GroupDocs.Merger.Mcp --prerelease --yes   # latest including pre-releases
```
...
The cache lives at `%USERPROFILE%\.nuget\packages\groupdocs.merger.mcp\<version>\`
on Windows and `~/.nuget/packages/groupdocs.merger.mcp/<version>/` on
...
info: ModelContextProtocol.Server.StdioServerTransport[...]
      Server (stream) (GroupDocs.Merger.Mcp) transport reading messages.
...
(
  ...
  sleep 2
) | GROUPDOCS_MCP_STORAGE_PATH=./docs dnx GroupDocs.Merger.Mcp@26.5.0 --yes
```

You should see two JSON-RPC responses containing `merge` and
`split`.
...
```bash
dotnet tool update -g GroupDocs.Merger.Mcp
```

To uninstall:

```bash
dotnet tool uninstall -g GroupDocs.Merger.Mcp
```

The tool runs from `~/.dotnet/tools/groupdocs-merger-mcp` (Linux/macOS) or
`%USERPROFILE%\.dotnet\tools\groupdocs-merger-mcp.exe` (Windows). Make sure
...
| `GROUPDOCS_LICENSE_PATH` | Path to `GroupDocs.Total.lic` — required for `split` | *(evaluation mode)* |
...
All three tools (`merge`, `split`, `get_document_info`) work in evaluation
mode without a GroupDocs license. `merge` and `split` still produce their
output files — the engine simply stamps an evaluation watermark on them and
the tool response carries an `[Evaluation mode] Output may include
watermarks.` banner.

Get a license from [purchase.groupdocs.com](https://purchase.groupdocs.com/)
to suppress the watermark. Point `GROUPDOCS_LICENSE_PATH` at the `.lic` file
and the response drops the banner.
...
```text
initialize response → serverInfo: { name: "GroupDocs.Merger.Mcp", version: "26.5.0" }
```
...
| `No license configured. Running in evaluation mode.` | No `GROUPDOCS_LICENSE_PATH` | Expected. All tools still work; merged / split output gets a watermark. Set the path to suppress it. |
| `[Evaluation mode] Output may include watermarks.` banner | No license | Expected. Set `GROUPDOCS_LICENSE_PATH` to suppress. |
| `<Op> failed for '<file>': System.DllNotFoundException` (Linux) | Missing `libgdiplus` / `libfontconfig1` | Install both via apt — pattern documented in the Tests-repo `.github/workflows/integration.yml`. |
