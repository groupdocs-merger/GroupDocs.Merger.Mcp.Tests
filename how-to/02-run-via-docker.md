...
- `ghcr.io/groupdocs-merger/merger-net-mcp` — GitHub Container Registry (primary)
- `docker.io/groupdocs/merger-net-mcp` — Docker Hub (mirror)
...
```bash
docker pull ghcr.io/groupdocs-merger/merger-net-mcp:26.4.4   # pinned to exact version
docker pull ghcr.io/groupdocs-merger/merger-net-mcp:latest   # floats to most recent push
```
...
docker pull ghcr.io/groupdocs-merger/merger-net-mcp:latest
...
```bash
docker inspect ghcr.io/groupdocs-merger/merger-net-mcp:latest \
...
(
  ...
  sleep 2
    ...
    -v $(pwd)/documents:/data \
    ghcr.io/groupdocs-merger/merger-net-mcp:26.4.4 2>/dev/null
```

Expected: two JSON-RPC responses on stdout. The second includes `merge`
and `split` with their descriptions and input schemas.
...
services:
  groupdocs-merger-mcp:
    image: ghcr.io/groupdocs-merger/merger-net-mcp:26.4.4
    stdin_open: true
    tty: false
    environment:
...
Without the license, `split` returns the evaluation-mode error
...
docker inspect ghcr.io/groupdocs-merger/merger-net-mcp:26.4.4 \
...
| `manifest unknown` / can't pull image | Version tag doesn't exist on that registry | Check [ghcr.io/groupdocs-merger/merger-net-mcp](https://github.com/groupdocs-merger/GroupDocs.Merger.Mcp/pkgs/container/merger-net-mcp) for available tags. |
| `:latest` ran but didn't pick up a new release | Docker reuses cached image — `docker run :latest` does not auto-pull | `docker pull ghcr.io/groupdocs-merger/merger-net-mcp:latest` before running, or add `--pull always` to your `docker run`. |
