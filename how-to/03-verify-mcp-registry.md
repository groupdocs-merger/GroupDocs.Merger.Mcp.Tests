...
The `GroupDocs.Merger.Mcp` package is an **MCP server** (the csproj declares
...
[nuget.org/packages/GroupDocs.Merger.Mcp](https://www.nuget.org/packages/GroupDocs.Merger.Mcp)
...
```bash
curl -L -o pkg.nupkg \
  https://www.nuget.org/api/v2/package/GroupDocs.Merger.Mcp/26.4.4
...
```json
{
  ...
  "name": "io.github.groupdocs-merger/groupdocs-merger-mcp",
  "version": "26.4.4",
  "packages": [ { "registryType": "nuget", "identifier": "GroupDocs.Merger.Mcp", ... } ],
  "repository": { "url": "https://github.com/groupdocs-merger/GroupDocs.Merger.Mcp" }
}
```
...
3. Search for `groupdocs-merger` or `merger`.
...
- `dnx` successfully resolves `GroupDocs.Merger.Mcp@26.4.4` from nuget.org.
...
curl -s https://registry.modelcontextprotocol.io/v0/servers \
  | jq '.[] | select(.name | contains("groupdocs-merger"))'
```
...
- `ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsMergerMcp` — the server
  reports `serverInfo.name == "GroupDocs.Merger.Mcp"` and a non-empty version
  that matches the package under test.
- `ToolDiscoveryTests.ListTools_ExposesReadAndSplit` — both MCP tools
  are advertised.
