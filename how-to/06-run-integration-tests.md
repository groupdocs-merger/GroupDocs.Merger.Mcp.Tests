...
This repo's test suite validates the **published** `GroupDocs.Merger.Mcp`
...
Three tests in `SplitTests` only assert success when a GroupDocs
...
# Only merge tests
dotnet test -c Release --filter "FullyQualifiedName~Merge"

# Only split tests
dotnet test -c Release --filter "FullyQualifiedName~Split"
...
Passed  GetDocumentInfoTests.GetDocumentInfo_TenPageDocx_ReportsTenPages
...
Total: ~18 (including parameterised cases), Passed: all, Time: ~30s
```
...
Synthetic PDF/JPEG fixtures ([SampleDocuments.cs](../src/GroupDocs.Merger.Mcp.Tests/Fixtures/SampleDocuments.cs))
...
- name: Dispatch integration tests
  ...
  run: |
    gh api \
      repos/groupdocs-merger/GroupDocs.Merger.Mcp.Tests/dispatches \
...
base64 -w0 GroupDocs.Total.lic | gh secret set GROUPDOCS_LICENSE \
  --repo groupdocs-merger/GroupDocs.Merger.Mcp.Tests
```
...
test fails with a cryptic `"An error occurred invoking 'split'"`,
...
| All Split tests fail | Running unlicensed. Expected — `Split_InEvaluationMode_ReturnsErrorResponse` should pass, the `*_Licensed` tests no-op. If the eval-mode test also fails, check the package actually installed. |
