...
- All guides target `GroupDocs.Merger.Mcp@26.4.4`. Substitute a newer version
  freely — the interfaces haven't changed.
- Tools exposed on the wire are `merge` and `split` (snake_case).
- `split` requires a licensed GroupDocs.Total.lic — without one, the
