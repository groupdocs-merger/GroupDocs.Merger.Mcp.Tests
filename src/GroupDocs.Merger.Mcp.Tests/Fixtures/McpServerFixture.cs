...
/// Boots the published GroupDocs.Merger.Mcp NuGet via `dnx` as a child process,
...
public sealed class McpServerFixture : IAsyncLifetime
{
    public string StoragePath { get; } = Path.Combine(
        Path.GetTempPath(),
        $"gdmerger-mcp-it-{Guid.NewGuid():N}");
    ...
    public async Task InitializeAsync()
    {
        ...
        var packageSpec = PackageVersion.IsLatest
            ...
            : $"GroupDocs.Merger.Mcp@{PackageVersion.Value}";

        var transport = new StdioClientTransport(new StdioClientTransportOptions
        {
            Name = "groupdocs-merger-mcp",
